"""Tests for main.py — process_message, poll_new_messages, initial_scan."""
import pytest
...
class TestProcessMessage:
    def test_applies_matching_label(self):
        ...
        service = MagicMock()
        with patch("main.get_message_headers") as mock_headers, \
             patch("main.get_matching_labels") as mock_labels, \
             patch("main.apply_label") as mock_apply:
...
class TestPollNewMessages:
    def test_processes_new_inbox_message(self):
        from main import poll_new_messages
        service = MagicMock()
        with patch("main.list_history") as mock_history, \
             patch("main.process_message") as mock_process:
...
class TestInitialScan:
    ...
    def test_processes_all_pending_messages(self, tmp_path):
        ...
        with patch("main.list_messages", return_value=messages), \
    ...
    def test_skips_already_processed_messages(self, tmp_path):
        ...
        with patch("main.list_messages", return_value=messages), \
             patch("main.load_scan_checkpoint", return_value=(already_processed, 0)), \
             patch("main.process_message") as mock_process, \
             patch("main.save_scan_checkpoint"):
            initial_scan(service, str(tmp_path), [], {})
            assert mock_process.call_count == 1
    ...
    def test_reprocesses_all_when_known_senders_grew(self, tmp_path):
        ...
        with patch("main.list_messages", return_value=messages), \
             patch("main.load_scan_checkpoint", return_value=(already_processed, 5)), \
             patch("main.process_message") as mock_process, \
             patch("main.save_scan_checkpoint"):
    ...
    def test_saves_checkpoint_on_completion(self, tmp_path):
        ...
        with patch("main.list_messages", return_value=messages), \
             patch("main.load_scan_checkpoint", return_value=(set(), 0)), \
             patch("main.process_message"), \
             patch("main.save_scan_checkpoint") as mock_save:
            initial_scan(service, str(tmp_path), [], {})
    ...
    def test_stops_when_running_is_false(self, tmp_path):
        ...
        with patch("main.list_messages", return_value=messages), \
             patch("main.load_scan_checkpoint", return_value=(set(), 0)), \
             patch("main.process_message", side_effect=fake_process), \
             patch("main.save_scan_checkpoint"):
            initial_scan(service, str(tmp_path), [], {})
    ...
    def test_no_messages_to_process(self, tmp_path):
        from main import initial_scan
        service = MagicMock()
        with patch("main.list_messages", return_value=[]), \
             patch("main.load_scan_checkpoint", return_value=(set(), 0)), \
             patch("main.process_message") as mock_process, \
             patch("main.save_scan_checkpoint"):
            initial_scan(service, str(tmp_path), [], {})

...
class TestFullScan:
    def test_returns_recipients_from_sent_messages(self, tmp_path):
        from gmail_service import list_sent_recipients
        service = mock_service()
        messages = [{"id": "s1"}, {"id": "s2"}]
        with patch("gmail_service._load_recipients_cache", return_value=(set(), None, 0)), \
             patch("gmail_service.list_messages", return_value=messages), \
             patch("gmail_service.get_message", side_effect=[
                 make_message("s1", to="alice@example.com"),
                 make_message("s2", to="bob@example.com"),
             ]), \
             patch("gmail_service.get_profile", return_value={"historyId": "h99"}), \
             patch("gmail_service._save_recipients_cache") as mock_save:
    ...
    def test_extracts_cc_and_bcc(self, tmp_path):
        ...
        service = mock_service()
        with patch("gmail_service._load_recipients_cache", return_value=(set(), None, 0)), \
             patch("gmail_service.list_messages", return_value=[{"id": "s1"}]), \
             patch("gmail_service.get_message", return_value=make_message(
                 "s1",
                 to="alice@example.com",
                 cc="bob@example.com",
                 bcc="charlie@example.com",
             )), \
             patch("gmail_service.get_profile", return_value={"historyId": "h1"}), \
             patch("gmail_service._save_recipients_cache"):
            result = list_sent_recipients(service, str(tmp_path))
        assert "alice@example.com" in result
        assert "bob@example.com" in result
    ...
    def test_returns_lowercase_addresses(self, tmp_path):
        ...
        with patch("gmail_service._load_recipients_cache", return_value=(set(), None, 0)), \
             patch("gmail_service.list_messages", return_value=[{"id": "s1"}]), \
             patch("gmail_service.get_message", return_value=make_message("s1", to="Alice@Example.COM")), \
             patch("gmail_service.get_profile", return_value={"historyId": "h1"}), \
             patch("gmail_service._save_recipients_cache"):
            result = list_sent_recipients(service, str(tmp_path))
        assert "alice@example.com" in result
    ...
    def test_returns_sorted_list(self, tmp_path):
        ...
        service = mock_service()
        with patch("gmail_service._load_recipients_cache", return_value=(set(), None, 0)), \
             patch("gmail_service.list_messages", return_value=[{"id": "s1"}, {"id": "s2"}]), \
             patch("gmail_service.get_message", side_effect=[
    ...
    def test_no_sent_messages_returns_empty(self, tmp_path):
        ...
        service = mock_service()
        with patch("gmail_service._load_recipients_cache", return_value=(set(), None, 0)), \
             patch("gmail_service.list_messages", return_value=[]), \
             patch("gmail_service.get_profile", return_value={"historyId": "h1"}), \
             patch("gmail_service._save_recipients_cache"):
            result = list_sent_recipients(service, str(tmp_path))
        assert result == []
...
class TestIncrementalUpdate:
    def test_no_new_messages_returns_cached_recipients(self, tmp_path):
        ...
        with patch("gmail_service._load_recipients_cache", return_value=(cached, "h1", 0)), \
             patch("gmail_service.list_history", return_value=[]), \
             patch("gmail_service.get_profile", return_value={"historyId": "h2"}), \
             patch("gmail_service._save_recipients_cache"):
            result = list_sent_recipients(service, str(tmp_path))
        assert "alice@example.com" in result
        assert "bob@example.com" in result
    ...
    def test_merges_new_recipients_with_cached(self, tmp_path):
        ...
        with patch("gmail_service._load_recipients_cache", return_value=(cached, "h1", 0)), \
             patch("gmail_service.list_history", return_value=history_records), \
             patch("gmail_service.get_message", return_value=make_message("s2", to="bob@example.com")), \
             patch("gmail_service.get_profile", return_value={"historyId": "h2"}), \
             patch("gmail_service._save_recipients_cache"):
            result = list_sent_recipients(service, str(tmp_path))
        assert "alice@example.com" in result
    ...
    def test_ignores_non_sent_messages_in_history(self, tmp_path):
        ...
        with patch("gmail_service._load_recipients_cache", return_value=(cached, "h1", 0)), \
             patch("gmail_service.list_history", return_value=history_records), \
             patch("gmail_service.get_message") as mock_get_msg, \
             patch("gmail_service.get_profile", return_value={"historyId": "h2"}), \
             patch("gmail_service._save_recipients_cache"):
            result = list_sent_recipients(service, str(tmp_path))
    ...
    def test_expired_history_id_falls_back_to_full_scan(self, tmp_path):
        ...
        service = mock_service()
        cached = {"alice@example.com"}
        with patch("gmail_service._load_recipients_cache", return_value=(cached, "expired_h", 0)), \
             patch("gmail_service.list_history", side_effect=Exception("404: historyId invalid")), \
             patch("gmail_service.list_messages", return_value=[{"id": "s1"}]), \
             patch("gmail_service.get_message", return_value=make_message("s1", to="bob@example.com")), \
             patch("gmail_service.get_profile", return_value={"historyId": "h_new"}), \
             patch("gmail_service._save_recipients_cache"):
            result = list_sent_recipients(service, str(tmp_path))
...
class TestInterruptionAndResume:
    def test_interrupted_scan_saves_progress(self, tmp_path):
        ...
        with patch("gmail_service._load_recipients_cache", return_value=(set(), None, 0)), \
             patch("gmail_service.list_messages", return_value=messages), \
             patch("gmail_service.get_message", return_value=make_message("sx", to="alice@example.com")), \
             patch("gmail_service._save_recipients_cache") as mock_save:
    ...
    def test_resumes_from_saved_index(self, tmp_path):
        ...
        with patch("gmail_service._load_recipients_cache", return_value=(set(), None, 2)), \
             patch("gmail_service.list_messages", return_value=messages), \
             patch("gmail_service.get_message", side_effect=fake_get_message), \
             patch("gmail_service.get_profile", return_value={"historyId": "h1"}), \
             patch("gmail_service._save_recipients_cache"):
            list_sent_recipients(service, str(tmp_path))
